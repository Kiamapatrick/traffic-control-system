...
def sample_network():
    return Network(
        junctions=[
            ...
            Junction(id="D", position=(0, 100), external_flow=-60),
        ],
        roads=[
...
class TestSolveEndpoint:
    @pytest.mark.asyncio
    async def test_solve_rref(self, client, sample_network):
        request = {
            "network": sample_network.model_dump(),
            "method": "rref"
        }
        response = await client.post("/api/v1/solve", json=request)
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_solve_lp(self, client, sample_network):
        request = {
            "network": sample_network.model_dump(),
            "method": "lp",
            "objective": "min_cost"
    ...
    @pytest.mark.asyncio
    async def test_solve_max_flow(self, client, sample_network):
        request = {
            "network": sample_network.model_dump(),
            "method": "max_flow",
    ...
    @pytest.mark.asyncio
    async def test_solve_invalid_method(self, client, sample_network):
        request = {
            "network": sample_network.model_dump(),
    ...
    @pytest.mark.asyncio
    async def test_solve_missing_params_max_flow(self, client, sample_network):
        request = {
            "network": sample_network.model_dump(),
            "method": "max_flow",
            "parameters": {}
...
class TestNetworkCRUD:
    @pytest.mark.asyncio
    async def test_create_network(self, client, sample_network):
        request = {
            "name": "Test Network",
            "network": sample_network.model_dump(),
            "description": "A test network"
        }
        response = await client.post("/api/v1/networks", json=request)
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_get_network(self, client, sample_network):
        create_request = {
            "name": "Test Network",
            "network": sample_network.model_dump(),
        }
        create_response = await client.post("/api/v1/networks", json=create_request)
    ...
    @pytest.mark.asyncio
    async def test_delete_network(self, client, sample_network):
        create_request = {
            "name": "Test Network",
            "network": sample_network.model_dump(),
        }
        ...
        response = await client.delete(f"/api/v1/networks/{network_id}")
        assert response.status_code == 200

        get_response = await client.get(f"/api/v1/networks/{network_id}")
